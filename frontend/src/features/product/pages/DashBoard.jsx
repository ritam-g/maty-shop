                            ...
                            return (
                                ...
                                >
                                    <div className="relative aspect-[4/5] overflow-hidden bg-slate-950">
                                        ...
                                        {/* Scroll container — closed before dots */}
                                        <div className="w-full h-full flex overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                            {validImages.length > 0 ? (
                                                validImages.map((imgUrl, imgIndex) => (
                                                    ...
                                                    <div key={imgIndex} className="w-full h-full shrink-0 snap-center relative">
                                                        <img
                                                            src={imgUrl}
                                                            alt={`${item.name} - File ${imgIndex + 1}`}
                                                ...
                                                <div className="w-full h-full shrink-0 snap-center bg-slate-800 flex items-center justify-center">
                                                    <span className="text-slate-500 font-semibold text-xs">No Image</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {validImages.length > 1 && (
                                            <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5 z-20 pointer-events-none">
                                                {validImages.map((_, dotIndex) => (
                                                    <div key={dotIndex} className="w-1.5 h-1.5 rounded-full bg-white/40 shadow-sm" />
                                                ))}
                                            </div>
                                        )}
